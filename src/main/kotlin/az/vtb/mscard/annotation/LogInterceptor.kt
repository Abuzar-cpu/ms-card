...
class LogInterceptor {
    ...
    @Before("@within(az.vtb.mscard.annotation.Log) && execution(* *(..))")
    ...
    @AfterThrowing("@within(az.vtb.mscard.annotation.Log) && execution(* *(..))")
    fun logError(joinPoint: JoinPoint) {
        val method = joinPoint.signature
        log.error { "ActionLog.${method.name}.error" }
    }

    @AfterReturning("@within(az.vtb.mscard.annotation.Log) && execution(* *(..))", returning = "result")
